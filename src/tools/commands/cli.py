...
def create_controller(name: str):
    ...
    # Read templates
    controller_template = read_template(
        "src/service_platform/commands/templates/controller_template.py",
    )
    manager_template = read_template(
        "src/service_platform/commands/templates/manager_template.py",
    )
    response_template = read_template(
        "src/service_platform/commands/templates/response_template.py",
    )
    schema_template = read_template(
        "src/service_platform/commands/templates/schema_template.py",
    )

    # Create the controller file with a basic template
    with open(controller_file_path, "w") as f:
        f.write(
            controller_template.format(name=name.capitalize(), name_lower=name.lower()),
        )

    # Create the manager file with a basic template
    with open(manager_file_path, "w") as f:
        f.write(
            manager_template.format(name=name.capitalize(), name_lower=name.lower()),
        )

    # Create the response file with a basic template
    with open(response_file_path, "w") as f:
        f.write(
            response_template.format(name=name.capitalize(), name_lower=name.lower()),
        )
...
def create_repository(name: str):
    ...
    # Read templates
    entity_template = read_template(
        "src/service_platform/commands/templates/entity_template.py",
    )
    repository_template = read_template(
        "src/service_platform/commands/templates/repository_template.py",
    )
    ...
    with open(repository_file_path, "w") as f:
        f.write(
            repository_template.format(name=name.capitalize(), name_lower=name.lower()),
        )
    ...
    typer.echo(
        f"Entity and Repository {name} created successfully at {entity_file_path} and {repository_file_path}.",
    )
